	...
	while (true){
	   ...
	   public static void Play(Player player,Gridn grid) {
		...
		while( grid.Fill(g[0],g[1])) {
			...
			System.out.println("Déja Rempli réesaye !");
			g[0] = getCase()[0]-1;
			g[1] =getCase()[1]-1;
			grid.Display();
			 StdDraw.pause(100);
		...
		grid.Insert(g[0],g[1],player.getPion());
	}
	

	public static void PlayIA(Player player,Gridn grid,int prof) {
		
			player.calculIA(grid, prof);
...
*/
	   ...
	   while (grid1.Verify()==-1) {
		   ...
		   if( grid1.Verify()==1 ) {System.out.print(player1.getName() +" à gagner ");break;}
		}else{
			
			System.out.print(player2.getName() +" C'est ton tour ! ");
			
			PlayIA(player2, grid1, 7);
			
			System.out.print(grid1.Display());
			grid1.disG();
			if( grid1.Verify()==2 ) {System.out.print(player2.getName() +" à gagner ");break;}
		}
